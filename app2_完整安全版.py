...
prompt_map = {
    ...
    "函式 (google sheet+excel函數公式)": "請列出適用於 Google Sheets 與 Excel 的實用函數，並提供簡要範例與用途（排除特休計算相關公式）。",
    "函式+解說 (google sheet+excel函數公式)": "請列出適用於 Google Sheets 與 Excel 的實用函數，並提供範例與說明其用途。",
    "教學生成(google sheet+excel函數公式)": "請針對適用的Google Sheets 與 Excel 的實用函數提供教學與實例。",
